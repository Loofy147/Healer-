...
static inline void fsc_syndromes_4way(const uint8_t* block, size_t n, __int128_t* s) {
    ...
    for (; i + 7 < n; i += 8) {
        ...
        v_s0 = _mm256_add_epi64(v_s0, _mm256_cvtepu32_epi64(_mm256_extracti128_si256(d32, 0)));
        v_s0 = _mm256_add_epi64(v_s0, _mm256_cvtepu32_epi64(_mm256_extracti128_si256(d32, 1)));
        __m256i p1 = _mm256_mullo_epi32(d32, v_w);
        ...
        __m256i p2 = _mm256_mullo_epi32(p1, v_w);
        ...
        __m256i p3 = _mm256_mullo_epi32(p2, v_w);
        v_s3 = _mm256_add_epi64(v_s3, _mm256_cvtepu32_epi64(_mm256_extracti128_si256(p3, 0)));
        v_s3 = _mm256_add_epi64(v_s3, _mm256_cvtepu32_epi64(_mm256_extracti128_si256(p3, 1)));
    ...
    s[0] = r[0]+r[1]+r[2]+r[3]; s[1] = r[4]+r[5]+r[6]+r[7];
    s[2] = r[8]+r[9]+r[10]+r[11]; s[3] = r[12]+r[13]+r[14]+r[15];
    for (; i < n; i++) {
